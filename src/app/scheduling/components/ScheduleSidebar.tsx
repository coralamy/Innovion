...
export default function ScheduleSidebar({ weekOffset, onWeekChange, jobs, contractors, todayDateStr }: ScheduleSidebarProps) {
    ...
    <div className="hidden xl:flex flex-col w-64 2xl:w-72 flex-shrink-0 border-l border-border bg-card overflow-y-auto scrollbar-thin">
      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between mb-3">
          ...
          <p className="text-sm font-700 text-foreground">{MONTHS[miniCalMonth]} {miniCalYear}</p>
          <button
            onClick={() => {
        ...
        <div className="grid grid-cols-7 mb-1">
          {SHORT_DAYS.map((d) => (
            <div key={`sh-${d}`} className="text-center text-xs font-600 text-muted-foreground py-1" style={{ fontSize: '10px' }}>
              {d}
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7">
          {calCells.map((dayNum, i) => {
            const isToday = dayNum === todayDay && miniCalMonth === todayMonth && miniCalYear === todayYear;
            ...
            return (
              <button
                ...
                style={{
                  fontSize: '11px',
                  backgroundColor: isToday ? 'var(--accent)' : isWeekDay ? 'rgba(37,99,235,0.1)' : 'transparent',
                  color: isToday ? 'white' : isWeekDay ? 'var(--accent)' : dayNum ? 'var(--foreground)' : 'transparent',
      ...
      <div className="p-4 border-b border-border space-y-3">
        <p className="text-xs font-700 uppercase tracking-widest text-muted-foreground" style={{ fontSize: '10px' }}>Today at a Glance</p>
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-sm font-700 text-success font-tabular">{todayJobs.filter(j => j.status === 'completed').length}</span>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">In progress</span>
          <span className="text-sm font-700 font-tabular" style={{ color: 'var(--info)' }}>{todayJobs.filter(j => j.status === 'in-progress').length}</span>
        </div>

        {conflictJobs.length > 0 && (
          <div className="flex items-center gap-2 p-2.5 rounded-lg border" style={{ backgroundColor: 'var(--danger-bg)', borderColor: 'rgba(239,68,68,0.25)' }}>
            <AlertTriangle size={13} className="text-danger flex-shrink-0" />
            <div>
              <p className="text-xs font-700 text-danger">{conflictJobs.length} conflict{conflictJobs.length > 1 ? 's' : ''}</p>
        ...
        {lateJobs.length > 0 && (
          <div className="flex items-center gap-2 p-2.5 rounded-lg border" style={{ backgroundColor: 'var(--warning-bg)', borderColor: 'rgba(245,158,11,0.25)' }}>
            <Clock size={13} className="text-warning flex-shrink-0" />
            <div>
              <p className="text-xs font-700 text-warning">{lateJobs.length} issue{lateJobs.length > 1 ? 's' : ''} flagged</p>
      ...
      <div className="p-4">
        <p className="text-xs font-700 uppercase tracking-widest text-muted-foreground mb-3" style={{ fontSize: '10px' }}>Contractor Status</p>
                ...
                <div key={c.id} className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">
                      {activeJob ? `On job · ${activeJob.site.split(' ')[0]}` : totalJobs > 0 ? `${totalJobs} job${totalJobs > 1 ? 's' : ''} today` : 'No jobs today'}
                    </p>
                  </div>
                  <span
                    className="w-2 h-2 rounded-full flex-shrink-0"
                    style={{
                      backgroundColor: activeJob ? 'var(--success)' : totalJobs > 0 ? 'var(--accent)' : 'var(--muted-foreground)',
                    }}
                  />
                </div>
              );
