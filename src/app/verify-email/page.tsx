      ...
      if (session?.user) {
        ...
        if (session.user.email_confirmed_at) {
          const isNewVerification =
            searchParams.get('type') === 'signup' || searchParams.get('verified') === 'true';
    ...
    <div className="w-full max-w-md text-center">
      ...
      {state === 'verified' && (
        <div className="py-8">
          ...
          <button
            ...
            className="mt-6 text-sm font-600 hover:underline"
      ...
      {state === 'pending' && (
        <div className="py-8">
          ...
          {resendError && (
            <div
              className="flex items-start gap-3 p-3 rounded-xl mt-4 border text-left"
              style={{ backgroundColor: '#fee2e2', borderColor: 'rgba(239,68,68,0.3)' }}
            >
              <AlertCircle
                size={14}
                ...
                style={{ color: '#ef4444' }}
              />
              <p className="text-xs font-500" style={{ color: '#ef4444' }}>
                {resendError}
              </p>
            </div>
          )}
          ...
          {resendSent ? (
            <div
              className="flex items-center justify-center gap-2 mt-5 p-3 rounded-xl"
              style={{ backgroundColor: '#dcfce7' }}
            >
              <CheckCircle2 size={16} style={{ color: '#22c55e' }} />
              <p className="text-sm font-600" style={{ color: '#16a34a' }}>
          ...
          <button
            ...
            className="mt-4 text-sm font-600 hover:underline text-muted-foreground"
          >
            ← Back to sign in
          </button>
        </div>
      ...
      {state === 'error' && (
        <div className="py-8">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            This verification link has expired or is invalid. Please sign up again or request a new
            verification email.
          </p>
          <button
            onClick={() => router.push('/sign-up-login')}
      ...
      <div className="hidden lg:flex lg:w-2/5 xl:w-1/2 flex-col justify-between p-10 xl:p-16 gradient-navy">
        ...
        <div>
          <h1 className="text-4xl xl:text-5xl font-800 text-white leading-tight">
            One step away
            <br />
            <span style={{ color: '#60A5FA' }}>from your</span>
            <br />
            workspace.
          </h1>
          <p className="text-lg mt-6 leading-relaxed" style={{ color: 'rgba(203,213,225,0.8)' }}>
            Verify your email to unlock full access to Innovion&apos;s intelligent operations
            platform.
          </p>
        </div>
