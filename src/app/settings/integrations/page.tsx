...
import {
  Globe, CheckCircle2, AlertCircle, XCircle, Clock, ChevronRight,
  Settings, ExternalLink, ToggleLeft, ToggleRight, Save, X, Eye, EyeOff,
  RefreshCw, Plug, Shield, Zap,
} from 'lucide-react';
import {
...
function ConfigureModal({ provider, existing, onClose, onSaved, companyId }: ConfigureModalProps) {
  const [displayValues, setDisplayValues] = useState<Record<string, string>>(() => {
    ...
    provider.displayFields.forEach((f) => { init[f.key] = existing?.displayConfig?.[f.key] ?? ''; });
    return init;
  });
  const [secretValues, setSecretValues] = useState<Record<string, string>>(() => {
    const init: Record<string, string> = {};
    provider.secretFields.forEach((f) => { init[f.key] = ''; });
    return init;
  });
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: 'var(--accent)', opacity: 0.1 }}>
              <Plug size={18} style={{ color: 'var(--accent)' }} />
            </div>
            <div>
              <h2 className="text-sm font-700" style={{ color: 'var(--foreground)' }}>{provider.name}</h2>
              <p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>{CATEGORY_LABELS[provider.category]}</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg transition-all hover:bg-secondary" style={{ color: 'var(--muted-foreground)' }}>
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-5 max-h-[60vh] overflow-y-auto">
          <p className="text-sm" style={{ color: 'var(--muted-foreground)' }}>{provider.description}</p>

          {/* OAuth notice */}
          {provider.usesOAuth && (
            <div className="flex items-start gap-2 p-3 rounded-lg text-xs" style={{ backgroundColor: 'rgba(37,99,235,0.06)', border: '1px solid rgba(37,99,235,0.15)' }}>
              <Zap size={12} className="flex-shrink-0 mt-0.5" style={{ color: 'var(--accent)' }} />
              <span style={{ color: 'var(--muted-foreground)' }}>
                This provider uses OAuth 2.0. Connection is initiated via the secure OAuth flow — no credentials are entered manually.
                All OAuth endpoints use <strong style={{ color: 'var(--foreground)' }}>innovion.app</strong> domain.
              </span>
            </div>
          )}

          {/* Enable toggle */}
          <div className="flex items-center justify-between py-2 rounded-lg px-3" style={{ backgroundColor: 'var(--secondary)' }}>
            <span className="text-sm font-600" style={{ color: 'var(--foreground)' }}>Enable Integration</span>
            <button onClick={() => setIsEnabled(!isEnabled)} style={{ color: isEnabled ? 'var(--accent)' : 'var(--muted-foreground)' }}>
          ...
          {provider.displayFields.length > 0 && (
            <div className="space-y-3">
              <h3 className="text-xs font-700 uppercase tracking-wide" style={{ color: 'var(--muted-foreground)', fontSize: '11px' }}>Configuration</h3>
              {provider.displayFields.map((field) => (
                <div key={field.key}>
                  <label className="block text-xs font-600 mb-1.5" style={{ color: 'var(--muted-foreground)', fontSize: '11px' }}>{field.label}</label>
                  <input
                    type="text"
                    value={displayValues[field.key] ?? ''}
                    onChange={(e) => setDisplayValues((p) => ({ ...p, [field.key]: e.target.value }))}
          ...
          {provider.secretFields.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-1.5">
                ...
                <h3 className="text-xs font-700 uppercase tracking-wide" style={{ color: 'var(--muted-foreground)', fontSize: '11px' }}>Credentials (encrypted at rest)</h3>
              </div>
                ...
                <div key={field.key}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) => setSecretValues((p) => ({ ...p, [field.key]: e.target.value }))}
        ...
        {/* Footer */}
        <div className="flex items-center justify-between px-6 py-4 gap-3" style={{ borderTop: '1px solid var(--border)' }}>
          <a
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <h3 className="text-sm font-700 truncate" style={{ color: 'var(--foreground)' }}>{provider.name}</h3>
              <span
              ...
              {provider.comingSoon && (
                <span
                  className="px-2 py-0.5 rounded-full text-xs font-600 flex-shrink-0"
                  style={{ backgroundColor: 'rgba(245,158,11,0.1)', color: 'var(--warning)', border: '1px solid rgba(245,158,11,0.2)' }}
                >
                  Coming Soon
                </span>
              )}
            </div>
            <p className="text-xs leading-relaxed line-clamp-3" style={{ color: 'var(--muted-foreground)', opacity: 0.9 }}>{provider.description}</p>
          </div>
        </div>
      </div>
      ...
      >
        ...
        {provider.comingSoon ? (
          <span className="text-xs font-500" style={{ color: 'var(--muted-foreground)' }}>Coming soon</span>
        ) : canManage ? (
          <button
            onClick={onConfigure}
          ...
          >
            {isActive ? <><Settings size={12} />Configure</> : <><Plug size={12} />Connect</>}
          </button>
        ) : (
          <span className="text-xs font-500" style={{ color: 'var(--muted-foreground)' }}>Admin only</span>
        )}
      </div>

      {integration?.lastError && (
        <div className="flex items-start gap-2 px-5 pb-3 text-xs" style={{ color: 'var(--danger)' }}>
          <AlertCircle size={12} className="flex-shrink-0 mt-0.5" />
          <span style={{ backgroundColor: 'rgba(239,68,68,0.08)', padding: '0.375rem 0.5rem', borderRadius: '0.375rem', display: 'block', width: '100%' }}>
            {integration.lastError}
          </span>
        </div>
      )}
...
export default function IntegrationsPage() {
  ...
  const activeCount = integrations.filter((i) => i.status === 'active' || i.status === 'configured').length;
    ...
    <AppLayout currentPath="/settings/integrations">
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="page-header-subtitle">
              Connect approved provider services to extend Platform capabilities.
        ...
        >
          ...
          <div>
            <p className="font-600" style={{ color: 'var(--foreground)' }}>Tenant-Isolated Integration Storage</p>
            <p style={{ color: 'var(--muted-foreground)' }}>
              All integration credentials are stored per-company with row-level security. Only administrators can configure integrations.
              Secret fields are stored encrypted and are never returned in API responses.
              All OAuth endpoints and external-facing URLs use the <strong style={{ color: 'var(--foreground)' }}>innovion.app</strong> domain.
            </p>
          </div>
        </div>
        ...
        {/* Error state */}
        {error && (
          <div className="flex items-center gap-2 p-4 rounded-xl text-sm" style={{ backgroundColor: 'rgba(239,68,68,0.08)', color: 'var(--danger)', border: '1px solid rgba(239,68,68,0.2)' }}>
            <AlertCircle size={16} />
            {error}
          </div>
        )}
        ...
        {!loading && filteredProviders.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <Globe size={40} className="mb-4" style={{ color: 'var(--muted-foreground)', opacity: 0.4 }} />
            <p className="text-sm font-600" style={{ color: 'var(--foreground)' }}>No providers in this category</p>
            <p className="text-xs mt-1" style={{ color: 'var(--muted-foreground)' }}>Select a different category or view all providers.</p>
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--muted-foreground)' }}>
          ...
          <span className="font-600" style={{ color: 'var(--foreground)' }}>Integrations</span>
        </div>
      </div>
