  ...
  return (
    <div className="card-elevated p-5 h-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="status-badge" style={{ backgroundColor: 'var(--info-bg)', color: 'var(--info)' }}>
          This Week
        </span>
      </div>
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={data ?? []} barGap={2} barSize={14}>
            ...
            <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: 'var(--muted-foreground)', fontWeight: 500 }} />
            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: 'var(--muted-foreground)' }} width={28} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(0,0,0,0.03)' }} />
            <Legend wrapperStyle={{ fontSize: '12px', paddingTop: '12px' }} formatter={(value) => <span style={{ color: 'var(--muted-foreground)', fontWeight: 500 }}>{value}</span>} />
            <Bar dataKey="scheduled" name="Scheduled" fill="var(--accent)" radius={[3, 3, 0, 0]} opacity={0.4} />
