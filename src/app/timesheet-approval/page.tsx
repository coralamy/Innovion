...
import { Clock, CheckCircle2, XCircle, AlertTriangle, FileText, Download, Search, Loader2, Send, Lock, X } from 'lucide-react';
...
const STATUS_CONFIG: Record<ApprovalStatus, { label: string; color: string; bg: string; icon: React.ElementType }> = {
  ...
  correction_required: { label: 'Correction Required', color: '#8B5CF6', bg: 'rgba(139,92,246,0.1)', icon: AlertTriangle },
};
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
      <div className="card-elevated w-full max-w-md p-6 rounded-2xl animate-slide-up space-y-4">
        ...
        <div className="p-3 rounded-lg text-sm" style={{ backgroundColor: 'var(--secondary)' }}>
          ...
          <p className="text-muted-foreground">{entry.job} · {entry.date} · {entry.totalHours?.toFixed(2)}h</p>
        </div>
        <div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            style={{ backgroundColor: action === 'approve' ? 'var(--success)' : action === 'reject' ? 'var(--danger)' : 'var(--accent)' }}
          >
            {action === 'approve' ? 'Approve' : action === 'reject' ? 'Reject' : action === 'correction' ? 'Send Back' : 'Reopen'}
          </button>
        </div>
      </div>
    </div>
            ...
            logs.map((log) => (
              <div key={log.id} className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {log.notes && <p className="text-xs text-muted-foreground mt-0.5 italic">&ldquo;{log.notes}&rdquo;</p>}
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {new Date(log.createdAt).toLocaleString('en-AU', { dateStyle: 'medium', timeStyle: 'short' })}
                  </p>
                </div>
              </div>
            ))
...
export default function TimesheetApprovalPage() {
  ...
  const [actionModal, setActionModal] = useState<{ entry: TimesheetEntry; action: 'approve' | 'reject' | 'correction' | 'reopen' } | null>(null);
  ...
  const logAudit = async (entryId: string, action: string, oldStatus: string, newStatus: string, notes: string) => {
    const supabase = createClient();
  ...
  const handleAction = async (notes: string) => {
    ...
    if (action === 'approve') {
      newStatus = 'approved';
      updates = { approval_status: 'approved', approved_at: now, approved_by: userName, approver_id: user?.id, is_locked: true };
      ...
      updates = { approval_status: 'in_progress', is_locked: false, approved_at: null, approved_by: null };
  ...
  const handleSubmit = async (entry: TimesheetEntry) => {
    const supabase = createClient();
    await supabase.from('time_entries').update({ approval_status: 'submitted', submitted_at: new Date().toISOString() }).eq('id', entry.id);
  ...
  const exportCSV = () => {
    ...
    const headers = ['Date', 'Contractor', 'Job', 'Site', 'Clock In', 'Clock Out', 'Break (min)', 'Total Hours', 'Hourly Rate', 'Amount', 'Approved By', 'Approved At'];
    const rows = approved.map((e) => [
      e.date, e.contractor, e.job, e.site, e.clockIn, e.clockOut || '', e.breakMinutes,
      e.totalHours?.toFixed(2) || '0', e.hourlyRate,
      ((e.totalHours || 0) * e.hourlyRate).toFixed(2),
      e.approvedBy || '', e.approvedAt ? new Date(e.approvedAt).toLocaleDateString('en-AU') : '',
    ]);
  ...
  const filtered = entries.filter((e) => {
    const matchSearch = !search || e.contractor.toLowerCase().includes(search.toLowerCase()) || e.job.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { label: 'Correction Required', value: stats.correction, color: '#8B5CF6', icon: AlertTriangle },
          ].map((s) => (
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {paginated.map((entry) => (
                    <tr key={entry.id} className="border-b hover:bg-secondary/30 transition-colors" style={{ borderColor: 'var(--border)' }}>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            {entry.isLocked && <span className="inline-flex items-center gap-0.5 text-xs" style={{ color: 'var(--muted-foreground)' }}><Lock size={10} /> Locked</span>}
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-xs font-600 text-foreground font-tabular">{entry.totalHours?.toFixed(2) || '—'}h</td>
                      <td className="px-4 py-3"><StatusBadge status={entry.approvalStatus} /></td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          {/* Submit (contractor/supervisor) */}
                          {entry.approvalStatus === 'in_progress' && !entry.isLocked && (
                            <button onClick={() => handleSubmit(entry)} className="px-2 py-1 rounded text-xs font-600 transition-colors hover:opacity-80" style={{ backgroundColor: 'rgba(37,99,235,0.1)', color: '#2563EB' }}>
                              Submit
                            </button>
                          )}
                          ...
                          {canApprove && entry.approvalStatus === 'submitted' && (
                            <>
                              <button onClick={() => setActionModal({ entry, action: 'approve' })} className="px-2 py-1 rounded text-xs font-600 transition-colors hover:opacity-80" style={{ backgroundColor: 'rgba(16,185,129,0.1)', color: '#10B981' }}>
                                Approve
                              </button>
                              <button onClick={() => setActionModal({ entry, action: 'reject' })} className="px-2 py-1 rounded text-xs font-600 transition-colors hover:opacity-80" style={{ backgroundColor: 'rgba(239,68,68,0.1)', color: '#EF4444' }}>
                                Reject
                              </button>
                              <button onClick={() => setActionModal({ entry, action: 'correction' })} className="px-2 py-1 rounded text-xs font-600 transition-colors hover:opacity-80" style={{ backgroundColor: 'rgba(139,92,246,0.1)', color: '#8B5CF6' }}>
                                Correct
                              </button>
                            </>
                          )}
                          ...
                          {isAdmin && entry.isLocked && (
                            <button onClick={() => setActionModal({ entry, action: 'reopen' })} className="px-2 py-1 rounded text-xs font-600 transition-colors hover:opacity-80" style={{ backgroundColor: 'rgba(245,158,11,0.1)', color: '#F59E0B' }}>
                              Reopen
                            </button>
                          )}
                          {/* Audit log */}
                          <button onClick={() => setAuditEntry(entry.id)} className="p-1.5 rounded hover:bg-secondary transition-colors" title="Audit Log">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="px-3 py-1.5 rounded text-xs font-600 border disabled:opacity-40 hover:bg-secondary transition-colors" style={{ borderColor: 'var(--border)' }}>Prev</button>
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                const p = Math.max(1, Math.min(page - 2, totalPages - 4)) + i;
                return (
                  <button key={p} onClick={() => setPage(p)} className="px-3 py-1.5 rounded text-xs font-600 border transition-colors" style={{ borderColor: 'var(--border)', backgroundColor: p === page ? 'var(--accent)' : 'transparent', color: p === page ? 'white' : 'var(--foreground)' }}>{p}</button>
                );
              })}
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="px-3 py-1.5 rounded text-xs font-600 border disabled:opacity-40 hover:bg-secondary transition-colors" style={{ borderColor: 'var(--border)' }}>Next</button>
            </div>
          </div>
        )}
      </div>
