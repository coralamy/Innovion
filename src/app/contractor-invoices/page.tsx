    ...
    >
      <div className="card-elevated w-full max-w-2xl rounded-2xl overflow-hidden animate-slide-up max-h-[90vh] flex flex-col">
        ...
        >
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              {invoice.invoiceNumber} · Issued by {invoice.contractorName}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          >
            <strong>Contractor Tax Invoice:</strong> This invoice is issued by{' '}
            {invoice.contractorName}. It is NOT a Recipient Created Tax Invoice (RCTI). The
            contractor is responsible for issuing this invoice.
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--secondary)' }}>
              ...
              {invoice.contractorAbn && (
                <p className="text-xs text-muted-foreground">ABN: {invoice.contractorAbn}</p>
              )}
              {invoice.contractorEmail && (
                <p className="text-xs text-muted-foreground">{invoice.contractorEmail}</p>
              )}
            </div>
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--secondary)' }}>
              ...
              <thead>
                <tr style={{ backgroundColor: 'var(--secondary)' }}>
                  {['Description', 'Date', 'Hours', 'Rate', 'Amount'].map((h) => (
                    <th
                      key={h}
                      className="text-left px-3 py-2.5 text-xs font-600 text-muted-foreground"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                <tr>
                  <td className="px-3 py-3 text-xs">
                    {invoice.job} — {invoice.site}
                  </td>
                  <td className="px-3 py-3 text-xs">{invoice.workDate}</td>
                  <td className="px-3 py-3 text-xs font-tabular">
                    {invoice.hoursWorked.toFixed(2)}
                  </td>
                  <td className="px-3 py-3 text-xs font-tabular">
                    ${invoice.hourlyRate.toFixed(2)}/hr
                  </td>
                  <td className="px-3 py-3 text-xs font-tabular font-600">
          ...
          <div className="ml-auto w-64 space-y-1.5">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-tabular">${invoice.subtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">GST (10%)</span>
              <span className="font-tabular">${invoice.gst.toFixed(2)}</span>
            </div>
            <div
              className="flex justify-between text-base font-700 border-t pt-2"
              style={{ borderColor: 'var(--border)' }}
            >
              <span>Total (AUD)</span>
              <span className="font-tabular">${invoice.total.toFixed(2)}</span>
            </div>
          </div>
            ...
            >
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm text-foreground">
                  Email invoice to company on my behalf
                </span>
              </label>
              {sendEmail && (
                <p className="text-xs text-muted-foreground pl-6">
                  By submitting, you authorise Innovion to send this invoice on your behalf. The
                  email will clearly identify you as the sender and this invoice as your Tax
                  Invoice.
                </p>
              )}
            </div>
        ...
        {/* Footer actions */}
        <div
          className="flex items-center gap-3 p-5 border-t flex-shrink-0"
          style={{ borderColor: 'var(--border)' }}
        >
          <button
            onClick={downloadPDF}
            className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-600 border transition-colors hover:bg-secondary"
            style={{ borderColor: 'var(--border)' }}
          >
            <Download size={15} />
            Download PDF
      ...
      <div className="space-y-6 animate-fade-in">
        <div>
          ...
          <p className="page-header-subtitle">
            Tax invoices generated from approved timesheets — issued by contractors
          </p>
        </div>
        ...
        <div className="filter-bar">
          <div className="relative flex-1 min-w-48">
            ...
            <input
              type="text"
              placeholder="Search contractor or invoice..."
              value={search}
              onChange={(e) => {
        ...
        <div className="card-elevated overflow-hidden">
          {loading ? (
            ...
            <div className="text-center py-16 text-muted-foreground">
              ...
              <p className="text-sm">
                No invoices yet. Approve timesheets to generate draft invoices.
              </p>
            </div>
          ) : (
                ...
                <tbody>
                  {paginated.map((inv) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs font-600 text-foreground">
                        {inv.contractorName}
                      </td>
                      ...
                      <td className="px-4 py-3 text-xs font-tabular">
                        {inv.hoursWorked.toFixed(2)}h
                      </td>
                      <td className="px-4 py-3 text-xs font-700 font-tabular text-foreground">
                        ${inv.total.toFixed(2)}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <button
                          ...
                          className="flex items-center gap-1.5 px-2.5 py-1 rounded text-xs font-600 transition-colors hover:opacity-80"
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button
                ...
                disabled={page === 1}
                className="px-3 py-1.5 rounded text-xs font-600 border disabled:opacity-40 hover:bg-secondary"
                style={{ borderColor: 'var(--border)' }}
              >
                Prev
              </button>
