...
export default function InventoryPage() {
  ...
  useEffect(() => {
    inventoryService.getAll(companyId).then((data) => {
      setInventory(data);
      setLoading(false);
    });
  }, [companyId]);
  ...
  const handleCreate = async () => {
    ...
    if (created) {
      ...
      setNewItem({ name: '', sku: '', supplier: '', location: '', unitCost: '', unit: 'units' });
    }
    setSaving(false);
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Track cleaning supplies, equipment, PPE, and consumables
            </p>
          </div>
            ...
            {canManage && (
              <>
                <PlannedAction
                  ...
                  title="Purchase ordering is not available yet."
                >
        ...
        <div className="card-elevated overflow-hidden">
          {loading ? (
            <div className="empty-state">
              <Package size={40} className="empty-state-icon animate-pulse" />
              <p className="empty-state-desc">Loading inventory...</p>
            </div>
          ) : (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div
                              ...
                              style={{ backgroundColor: `${categoryColors[item.category]}18` }}
                            >
                              <Package size={14} style={{ color: categoryColors[item.category] }} />
                            </div>
                            <div>
                            ...
                            <div className="relative">
                              ...
                              {openMenu === item.id && (
                                <div className="absolute right-0 top-8 z-20 w-40 card-elevated rounded-lg shadow-lg overflow-hidden animate-slide-up">
                                  {[
                                    { icon: Edit2, label: 'Edit Item' },
                                    { icon: RefreshCw, label: 'Restock' },
                                    { icon: ShoppingCart, label: 'Order More' },
                                    { icon: Trash2, label: 'Remove' },
                                  ].map((a) => (
                                    <button
                                      key={a.label}
                                      onClick={() => {
                                        if (a.label === 'Remove') handleDelete(item.id);
