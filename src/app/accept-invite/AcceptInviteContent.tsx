        ...
        <div className="card-elevated p-8 rounded-2xl">
          {error && step !== 'setup' ? (
            <div className="text-center space-y-4">
              <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto" style={{ backgroundColor: 'var(--danger-bg)' }}>
                <AlertCircle size={32} style={{ color: 'var(--danger)' }} />
              </div>
              <h1 className="text-xl font-700 text-foreground">Invitation Error</h1>
              <p className="text-sm text-muted-foreground">{error}</p>
              <button onClick={() => router.push('/sign-up-login')} className="w-full py-2.5 rounded-lg text-sm font-600 text-white transition-all hover:opacity-90" style={{ backgroundColor: 'var(--accent)' }}>
                Go to Login
              </button>
            </div>
          ) : step === 'success' ? (
            <div className="text-center space-y-4">
              <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto" style={{ backgroundColor: 'var(--success-bg)' }}>
                <CheckCircle2 size={32} style={{ color: 'var(--success)' }} />
              </div>
              <h1 className="text-xl font-700 text-foreground">Account Activated!</h1>
              <p className="text-sm text-muted-foreground">Your account has been created. Please check your email to verify your address, then sign in.</p>
              <button onClick={() => router.push('/sign-up-login')} className="w-full py-2.5 rounded-lg text-sm font-600 text-white transition-all hover:opacity-90" style={{ backgroundColor: 'var(--accent)' }}>
                Sign In
              </button>
            </div>
            ...
            <form onSubmit={handleActivate} className="space-y-5">
              <div className="text-center mb-2">
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  You&apos;ve been invited as <strong>{invite?.role}</strong>. Set up your account below.
                </p>
              </div>

              <div>
                <label className="block text-xs font-600 text-muted-foreground mb-1.5">Email Address</label>
                <input type="email" value={invite?.email || ''} readOnly className="w-full px-3 py-2.5 text-sm rounded-lg border bg-secondary opacity-70 cursor-not-allowed" style={{ borderColor: 'var(--border)' }} />
              </div>
              ...
              <div>
                ...
                <input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Your full name"
                  className="w-full px-3 py-2.5 text-sm rounded-lg border bg-background focus:outline-none focus:ring-2 focus:ring-accent/30" style={{ borderColor: 'var(--border)' }} required />
              </div>

              <div>
                <label className="block text-xs font-600 text-muted-foreground mb-1.5">Password</label>
                <div className="relative">
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min. 8 characters"
                    className="w-full px-3 py-2.5 pr-10 text-sm rounded-lg border bg-background focus:outline-none focus:ring-2 focus:ring-accent/30" style={{ borderColor: 'var(--border)' }} required />
              ...
              <div>
                <label className="block text-xs font-600 text-muted-foreground mb-1.5">Confirm Password</label>
              ...
              {error && (
                <div className="flex items-center gap-2 p-3 rounded-lg text-sm" style={{ backgroundColor: 'var(--danger-bg)', color: 'var(--danger)' }}>
