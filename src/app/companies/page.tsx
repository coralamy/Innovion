...
export default function CompaniesPage() {
  ...
  useEffect(() => {
    if (!canAccess) return;
    /**
     * DEFECT REMEDIATED (tenant scoping):
     *   `companyService.getAll()` was called with NO companyId, so the query
     *   was unscoped and relied entirely on RLS to decide what came back.
     *   Under the legacy `company_access_*` policy that included every row with
     *   a NULL company_id — i.e. the fictitious seed companies ("Crown Casino
     *   Complex", "Westfield Shopping Centre") shipped by migration
     *   20260726090000, shown to every tenant as though they were their own
     *   records. The tenant is now passed explicitly.
     */
    companyService.getAll(companyId).then((data) => {
      setCompanies(data);
      setLoading(false);
    });
  }, [canAccess, companyId]);
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    // The tenant MUST be supplied: without it the row is created with
    // company_id NULL, which RLS refuses and the NOT NULL constraint added by
    // migration 20260817004000 refuses as well.
    const created = await companyService.create(
      {
        ...
        logo: newCompany.name.slice(0, 2).toUpperCase(),
      },
      companyId
    );
    if (created) {
      ...
      setNewCompany({ name: '', industry: '', location: '', phone: '', email: '' });
    }
    setSaving(false);
  };
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="page-header-subtitle">
              Manage clients, contractors, and partner organisations
            </p>
          </div>
          <button
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input
                    ...
                    value={(newCompany as Record<string, string>)[f.key]}
                ...
                <thead>
                  ...
                  >
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left px-4 py-3 text-xs font-600 uppercase tracking-wide text-muted-foreground"
                        style={{ fontSize: '11px' }}
                      >
                        {h}
                      </th>
                      ...
                      >
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            <div
                              className="w-9 h-9 rounded-lg flex items-center justify-center text-xs font-700 text-white flex-shrink-0"
                              style={{ backgroundColor: 'var(--primary)' }}
                            >
                              {company.logo}
                            </div>
                            <div>
                              <p className="text-sm font-600 text-foreground">{company.name}</p>
                              ...
                              <div className="absolute right-0 top-8 z-20 w-40 card-elevated rounded-lg shadow-lg overflow-hidden animate-slide-up">
                                {[
                                  ...
                                  <button
                                    key={action.label}
                                    onClick={() => {
                                      if (action.label === 'Delete') handleDelete(company.id);
