...
export default function IncidentsPage() {
  ...
  const [newInc, setNewInc] = useState({
    ...
    reportedBy: '',
    assignedTo: '',
    severity: 'medium' as IncidentRecord['severity'],
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="page-header-subtitle">
              Log, track, and resolve workplace incidents and near-misses
            </p>
          </div>
          {canManage && (
            <button
          ...
          <div className="card-elevated p-5 space-y-4 animate-slide-up">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <div key={f.key}>
                  ...
                  <input
                    ...
                    value={(newInc as Record<string, string>)[f.key]}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewInc((p) => ({
                      ...p,
                      severity: e.target.value as IncidentRecord['severity'],
                    }))
                  }
              ...
              <div>
                ...
                <select
                  ...
                  value={newInc.type}
                  onChange={(e) =>
                    setNewInc((p) => ({ ...p, type: e.target.value as IncidentRecord['type'] }))
                  }
                  className="w-full px-3 py-2 text-sm rounded-lg border bg-background focus:outline-none"
                  style={{ borderColor: 'var(--border)' }}
                >
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  ...
                  <label
                    className="w-16 h-16 rounded-lg border-2 border-dashed flex flex-col items-center justify-center cursor-pointer hover:bg-secondary transition-colors"
                    style={{ borderColor: 'var(--border)' }}
                  >
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={saving}
                className="px-4 py-2 rounded-lg text-sm font-600 text-white disabled:opacity-60 flex items-center gap-2"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            {
              label: 'Investigating',
              value: loading ? '—' : stats.investigating,
              color: 'var(--warning)',
              icon: Clock,
            },
            {
            ...
            <div className="card-elevated p-3 flex flex-wrap items-center gap-3">
              <div className="flex-1 min-w-[180px] relative">
                ...
                <input
                  suppressHydrationWarning
                  type="text"
                  placeholder="Search incidents, sites..."
                  value={search}
                ...
                {paginated.map((incident) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ borderLeft: `3px solid ${isSelected ? 'var(--accent)' : sc.dot}` }}
                    >
                      <div className="flex items-start gap-3">
                        <div
                        ...
                        >
                          <AlertTriangle size={14} style={{ color: sc.text }} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {incident.photoUrls && incident.photoUrls.length > 0 && (
                              ...
                              >
                                <ImageIcon size={10} />
                                {incident.photoUrls.length} photo
                                {incident.photoUrls.length > 1 ? 's' : ''}
                              </span>
                            )}
                          </div>
              ...
              <div className="card-elevated p-5 space-y-4 sticky top-6 animate-slide-up">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      className="status-badge"
                      style={{
                        backgroundColor: severityConfig[selected.severity].bg,
                        color: severityConfig[selected.severity].text,
                      }}
                    >
                      {severityConfig[selected.severity].label} Severity
                    </span>
                    <span
                      className="status-badge"
                      style={{
                ...
                <div className="space-y-2 text-sm">
                  {[
                    ...
                    { label: 'Date reported', value: selected.reportedDate },
                    { label: 'Assigned to', value: selected.assignedTo },
                    { label: 'Resolved', value: selected.resolvedDate ?? 'Pending' },
                  ].map((row) => (
                    <div key={row.label} className="flex justify-between">
                      <span className="text-muted-foreground">{row.label}</span>
                      <span className="font-500 text-foreground text-right max-w-[60%]">
                        {row.value}
                      </span>
                    </div>
                  ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selected.photoUrls.map((url, idx) => (
                        <a
                          ...
                          className="w-16 h-16 rounded-lg overflow-hidden border block"
                          style={{ borderColor: 'var(--border)' }}
                        >
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                ...
                <div className="flex gap-2 pt-2">
                  {selected.status === 'open' && (
                    <button
                      onClick={() => handleUpdateStatus(selected.id, 'investigating')}
                      className="flex-1 py-2 rounded-lg text-sm font-600 text-white transition-all hover:opacity-90"
                  ...
                  {selected.status === 'resolved' && (
                    <button
                      ...
                      className="flex-1 py-2 rounded-lg text-sm font-600 transition-all hover:bg-secondary"
