...
export default function AppLayout({ children, currentPath = '/dashboard' }: AppLayoutProps) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(1)
      .then(({ data }) => {
        setProvisioned(!!(data && data.length > 0));
      })
      .catch(() => {
        // On error, fall back to metadata value (false → redirect to onboarding)
      ...
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4">
          <div className="relative">
            <div className="w-10 h-10 rounded-full border-2 border-border animate-pulse" style={{ borderColor: 'var(--border)' }} />
            <div className="absolute inset-0 w-10 h-10 rounded-full border-2 border-transparent border-t-accent animate-spin" style={{ borderTopColor: 'var(--accent)' }} />
          </div>
          <div className="flex flex-col items-center gap-1">
            <p className="text-sm font-600" style={{ color: 'var(--foreground)' }}>Loading workspace</p>
            <p className="text-xs" style={{ color: 'var(--muted-foreground)' }}>Please wait…</p>
          </div>
        </div>
      </div>
    );
