...
export const complianceService = {
  async getAll(companyId?: string | null): Promise<ComplianceRecord[]> {
    ...
    if (error) {
      logger.error('complianceService', 'Failed to fetch compliance items', {
        companyId,
        error: error.message,
      });
      return [];
  ...
  async create(
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        assigned_type: item.assignedType,
        comp_status: item.status,
      ...
      .single();
    if (error) {
      logger.error('complianceService', 'Failed to create compliance item', {
        title: item.title,
        error: error.message,
      });
      return null;
  ...
  async updateStatus(id: string, status: ComplianceRecord['status']): Promise<boolean> {
    ...
    const { error } = await supabase
      .from('compliance_items')
      .update({ comp_status: status, updated_at: new Date().toISOString() })
      .eq('id', id);
    if (error) {
      logger.error('complianceService', 'Failed to update compliance status', {
        id,
        status,
        error: error.message,
      });
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (error) {
      logger.error('complianceService', 'Failed to delete compliance item', {
        id,
        error: error.message,
      });
      return false;
