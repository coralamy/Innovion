...
export const siteService = {
  async getAll(companyId?: string | null): Promise<SiteRecord[]> {
    ...
    if (error) {
      logger.error('siteService', 'Failed to fetch sites', { companyId, error: error.message });
      return [];
    }
  ...
  async create(
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        area: site.area,
        floors: site.floors,
        active_jobs: site.activeJobs,
        last_service: site.lastService,
        next_service: site.nextService,
        site_status: site.status,
        assigned_team: site.assignedTeam,
      ...
      .single();
    if (error) {
      logger.error('siteService', 'Failed to create site', {
        name: site.name,
        error: error.message,
      });
      return null;
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (error) {
      logger.error('siteService', 'Failed to delete site', { id, error: error.message });
      return false;
    }
    return true;
  },
