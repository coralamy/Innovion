...
export const clientService = {
  async getAll(companyId?: string | null): Promise<ClientRecord[]> {
    ...
    if (error) {
      logger.error('clientService', 'Failed to fetch clients', { companyId, error: error.message });
      return [];
    }
  ...
  async create(
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        phone: client.phone,
        email: client.email,
        active_contracts: client.activeContracts,
      ...
      .single();
    if (error) {
      logger.error('clientService', 'Failed to create client', {
        name: client.name,
        error: error.message,
      });
      return null;
  ...
  async update(id: string, updates: Partial<ClientRecord>): Promise<ClientRecord | null> {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error) {
      logger.error('clientService', 'Failed to update client', { id, error: error.message });
      return null;
    }
    return rowToClient(data as ClientRow);
  },
  ...
  async delete(id: string): Promise<boolean> {
    ...
    if (error) {
      logger.error('clientService', 'Failed to delete client', { id, error: error.message });
      return false;
    }
    return true;
  },
