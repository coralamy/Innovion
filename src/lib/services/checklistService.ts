  ...
  async getAll(companyId?: string | null): Promise<Checklist[]> {
    ...
    try {
      ...
      if (error) {
        ...
        logger.warn('checklistService', 'Failed to fetch checklists', {
          companyId,
          error: error.message,
        });
        return [];
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          initials: checklist.initials,
          avatar_color: checklist.avatarColor,
          checklist_date: checklist.date,
          checklist_status: checklist.status,
          sections: checklist.sections,
          signed_off_by: checklist.signedOffBy,
          signed_off_at: checklist.signedOffAt,
      ...
      if (error) {
        if (isSchemaError(error)) throw error;
        logger.warn('checklistService', 'Failed to create checklist', {
          title: checklist.title,
          error: error.message,
        });
        return null;
      ...
      logger.error(
        'checklistService',
        'Schema error creating checklist',
        { title: checklist.title },
        err
      );
      throw err;
