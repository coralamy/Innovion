...
async function runNormalConnectivityTests(): Promise<void> {
  ...
  await syncTest('SYNC-CONN-01', 'Supabase REST API is reachable', async () => {
    ...
    if (res.status === 404 || res.status === 0) throw new Error(`Supabase REST not reachable: ${res.status}`);
  });
...
async function runOfflineOperationTests(): Promise<void> {
  ...
  await syncTest('SYNC-OFF-01', 'Sync queue infrastructure: activity_log table exists', async () => {
    const { status } = await supabaseRest('activity_log', 'GET', { select: 'id', limit: '1' });
    if (status === 404) throw new Error('activity_log table not found — sync queue infrastructure missing');
  });

  await syncTest('SYNC-OFF-02', 'Sync queue infrastructure: notifications table exists', async () => {
    const { status } = await supabaseRest('notifications', 'GET', { select: 'id', limit: '1' });
    if (status === 404) throw new Error('notifications table not found');
  });
  ...
  await syncTest('SYNC-RETRY-01', 'Exponential backoff logic validation (unit test)', async () => {
    ...
    for (let i = 1; i < delays.length - 1; i++) {
      ...
      if (currBase <= prevBase && currBase < MAX_DELAY_MS) {
        throw new Error(`Backoff not increasing: attempt ${i} base=${currBase} <= prev=${prevBase}`);
      }
    }
    ...
    if (maxDelay > MAX_DELAY_MS + JITTER_MAX_MS) {
      throw new Error(`Backoff exceeded max delay: ${maxDelay}ms > ${MAX_DELAY_MS + JITTER_MAX_MS}ms`);
    }
  });
...
async function runIdempotencyTests(): Promise<void> {
  ...
  await syncTest('SYNC-IDEM-01', 'Jobs table has UUID primary key (idempotent upsert capable)', async () => {
  ...
  await syncTest('SYNC-IDEM-02', 'Checklists table has UUID primary key (idempotent upsert capable)', async () => {
    const { status } = await supabaseRest('checklists', 'GET', { select: 'id', limit: '1' });
    if (status === 404) throw new Error('Checklists table not found');
  });

  await syncTest('SYNC-IDEM-03', 'Idempotency key pattern: ON CONFLICT DO NOTHING validation', async () => {
    ...
    const { status } = await supabaseRest('role_permissions', 'GET', { select: 'id', limit: '1' });
    if (status === 404) throw new Error('role_permissions table not found — idempotency infrastructure missing');
  });
...
async function runAuthExpiryTests(): Promise<void> {
  ...
  await syncTest('SYNC-AUTH-01', 'Expired JWT returns 401 from Supabase REST API', async () => {
    ...
    const expiredJwt = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJ0ZXN0IiwiZXhwIjoxfQ.invalid';
  ...
  await syncTest('SYNC-AUTH-02', 'SessionExpiredModal component is registered in AppLayout', async () => {
...
async function runConflictResolutionTests(): Promise<void> {
  ...
  await syncTest('SYNC-CONF-02', 'Checklists table has updated_at timestamp column', async () => {
    const { status } = await supabaseRest('checklists', 'GET', { select: 'updated_at', limit: '1' });
    if (status === 404) throw new Error('Checklists table not found');
  });

  await syncTest('SYNC-CONF-03', 'Conflict resolution policy: platform-wins-when-newer', async () => {
    ...
    if (!platformWins) throw new Error('Conflict resolution logic inverted — platform should win when newer');
    ...
    if (!workforceWins) throw new Error('Conflict resolution logic inverted — workforce should win when newer');
  });
...
async function runSyncHealthTests(): Promise<void> {
  ...
  await syncTest('SYNC-HEALTH-01', 'Activity log table supports sync health tracking', async () => {
    const { status } = await supabaseRest('activity_log', 'GET', { select: 'id,created_at', limit: '1' });
    if (status === 404) throw new Error('activity_log table not found — sync health tracking unavailable');
  });

  await syncTest('SYNC-HEALTH-02', 'Notifications table supports sync failure reporting', async () => {
    const { status } = await supabaseRest('notifications', 'GET', { select: 'id,created_at', limit: '1' });
    if (status === 404) throw new Error('notifications table not found');
  });
  ...
  await syncTest('SYNC-HEALTH-03', 'Sync health metrics schema validation', async () => {
    ...
    if (!SUPABASE_URL) throw new Error('Supabase not configured — cannot validate sync health infrastructure');
...
async function main(): Promise<void> {
  console.log(`\n${BOLD}${CYAN}╔══════════════════════════════════════════════════════════════════════╗${RESET}`);
  console.log(`${BOLD}${CYAN}║  INNOVION SYNC VALIDATION — PILOT READINESS BUILD PACKAGE STEP 2    ║${RESET}`);
