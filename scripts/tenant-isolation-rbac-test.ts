...
async function isolationTest(
  ...
  if (blockedReason) {
    results.push({ id, category: currentCategory, name, status: 'BLOCKED', severity, blockedReason });
    ...
    await isolationTest(
      ...
      async () => {
        ...
        if (Array.isArray(data) && data.length > 0) {
          throw new Error(`RLS FAILURE: ${data.length} records from ${table} returned without authentication`);
        }
      },
      severity
    );
...
async function runCrossTenantTests(): Promise<void> {
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (Array.isArray(data) && data.length > 0) {
        throw new Error(`CROSS-TENANT EXPOSURE: ${data.length} jobs returned for fabricated company_id`);
      }
    },
    'CRITICAL'
  );
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (Array.isArray(data) && data.length > 0) {
        throw new Error(`CROSS-TENANT EXPOSURE: ${data.length} employees returned for fabricated company_id`);
      }
    },
    'CRITICAL'
  );
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (status === 404) return;
      if (Array.isArray(data) && data.length > 0) {
        throw new Error(`CROSS-TENANT EXPOSURE: ${data.length} settings returned for fabricated company_id`);
      }
    },
    'CRITICAL'
  );
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (status === 404) return;
      if (Array.isArray(data) && data.length > 0) {
        throw new Error(`CROSS-TENANT EXPOSURE: ${data.length} documents returned for fabricated company_id`);
      }
    },
    'CRITICAL'
  );
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (status === 404) return;
      if (Array.isArray(data) && data.length > 0) {
        throw new Error(`CROSS-TENANT EXPOSURE: ${data.length} provider_integrations returned for fabricated company_id`);
      }
    },
    'CRITICAL'
  );
  ...
  await isolationTest(
    ...
    async () => { throw new Error('Requires live test users'); },
    'CRITICAL',
...
async function runRBACTests(): Promise<void> {
    ...
    async () => {
      ...
      for (const [role, perms] of Object.entries(rolePermissions)) {
        if (role === 'admin') {
          ...
          if (perms.canManageCompany) throw new Error(`${role} should NOT have canManageCompany=true`);
        }
      }
    },
    'CRITICAL'
  ...
  await isolationTest(
    ...
    async () => {
      const { status } = await supabaseRest('user_roles', 'POST', {}, {
  ...
  await isolationTest(
    ...
    async () => {
      const { status } = await supabaseRest('role_permissions', 'POST', {}, {
        company_id: '00000000-0000-0000-0000-000000000001',
  ...
  await isolationTest(
    ...
    async () => {
      const { status } = await supabaseRest('platform_api_keys', 'POST', {}, {
        company_id: '00000000-0000-0000-0000-000000000001',
        key_hash: 'test-hash',
        key_prefix: 'pk_test',
      });
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (supervisorInDefaults) {
        throw new Error('Supervisor should NOT be in DEFAULT_ROLE_PERMISSIONS — must always use DB');
      }
    },
    'HIGH'
  );
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (contractorPerms.canManageUsers) throw new Error('Contractor should not have canManageUsers');
      if (contractorPerms.canManageCompany) throw new Error('Contractor should not have canManageCompany');
      if (contractorPerms.canViewFinancials) throw new Error('Contractor should not have canViewFinancials');
    },
    'HIGH'
  );
  ...
  await isolationTest(
    ...
    async () => {
      ...
      for (const table of tables) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          throw new Error(`CROSS-TENANT: ${data.length} ${table} records returned for fabricated company_id`);
        }
      }
    },
    'CRITICAL'
...
async function runStorageAccessTests(): Promise<void> {
  ...
  await isolationTest(
    ...
    async () => {
      ...
      if (res.status === 200) {
        warnLog('Storage returned 200 for unauthenticated request — verify bucket policy in Supabase dashboard');
  ...
  await isolationTest(
    ...
    async () => { throw new Error('Requires authenticated test users'); },
    'CRITICAL',
...
async function runAuditAttributionTests(): Promise<void> {
  ...
  await isolationTest(
    ...
    async () => {
      ...
      const { status } = await supabaseRest('activity_log', 'POST', {}, {
        user_id: '00000000-0000-0000-0000-000000000001',
        company_id: '00000000-0000-0000-0000-000000000001',
        action: 'test_action',
        entity_type: 'test',
  ...
  await isolationTest(
    ...
    async () => { throw new Error('Requires authenticated test user'); },
    'HIGH',
...
function printIsolationSummary(): void {
  ...
  console.log(`  ${RED}Failed:  ${failed}${RESET} (Critical: ${criticalFails}, High: ${highFails})`);
...
async function main(): Promise<void> {
  console.log(`\n${BOLD}${CYAN}╔══════════════════════════════════════════════════════════════════════╗${RESET}`);
  console.log(`${BOLD}${CYAN}║  INNOVION TENANT ISOLATION & RBAC — PILOT READINESS STEP 3          ║${RESET}`);
